"""
Semantic relevance checker — verifies that a with_example_related_to_exercise component
is genuinely anchored in both the exercise context and the knowledge component.
...
def _is_declared_function_kc(kc_name: str, kc_description: str) -> bool:
    """True when the KC is about a student-declared function (FO.2.x or FO.4.2.x)."""
...
def _is_native_function_kc(kc_name: str, kc_description: str) -> bool:
    """True when the KC is about a native/predefined platform function (FO.4.1.x)."""
    name_upper = kc_name.upper()
    desc_lower = kc_description.lower()
    return (
...
def _native_def_in_content(content: str) -> bool:
    """Fast pre-check: does the content define a native primitive with def?"""
...
class RelevanceChecker:
    ...
    async def check(
        ...
        and complies with the active platform configuration rules.
        """
        # Fast path: native primitive defined with def — reject immediately
        ...
        # Fast path: forbidden vocabulary in config
        if platform_config:
        ...
        log_prompt(run_id, "relevance_checker", user=user, system=_SYSTEM)
...
def _build_exercise_block(
    ...
    # Append exercise-specific section from platform context if available
